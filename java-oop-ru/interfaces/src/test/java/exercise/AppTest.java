...
class AppTest {
    ...
    @BeforeAll
    static void init() {
        text = new ReversedSequence("test");
    }

    @Test
    void testReverseSequenceToString() {
        var expected =  "tset";

        String result = text.toString();

        assertThat(result).as("Text is reversed").isEqualTo(expected);
    }

    @Test
    void testReverseSequenceCharAt() {
        var expected =  's';

        var result = text.charAt(1);

        assertThat(result).as("Char is").isEqualTo(expected);
    }

    @Test
    void testReverseSequenceLength() {
        var expected =  4;

        var result = text.length();

        assertThat(result).as("Length text is").isEqualTo(expected);
    }

    @Test
    void testReverseSequenceSubSequence() {
        var expected =  "te";

        var result = text.subSequence(1, 3);

        assertThat(result).as("Sequence text is").isEqualTo(expected);
    }
    // END
}
